...
"#;
    ...
    // Granularly add Caelestia without touching other keys
    ini.set("kwin", "CaelestiaLauncher", "Meta+Space,none,Caelestia Launcher");
    assert_eq!(ini.get("kwin", "CaelestiaLauncher").as_deref(), Some("Meta+Space,none,Caelestia Launcher"));

    // Granularly remove Caelestia
    let removed = ini.remove("kwin", "CaelestiaLauncher");
    assert_eq!(removed.as_deref(), Some("Meta+Space,none,Caelestia Launcher"));
    assert_eq!(ini.get("kwin", "CaelestiaLauncher"), None);
...
"#;
    ...
    let backup_dir = mock_data.join("caelestia").join("shortcuts-backup");

    std::env::set_var("XDG_CONFIG_HOME", &mock_config);
    std::env::set_var("XDG_DATA_HOME", &mock_data);
    std::env::set_var("CAELESTIA_SHORTCUTS_BACKUP_DIR", &backup_dir);
    ...
    // Verify Caelestia shortcuts were added
    let current_ini = KdeIniFile::parse(&fs::read_to_string(&kglobal_path).unwrap());
    assert_eq!(
        current_ini.get("kwin", "CaelestiaLauncher").as_deref(),
        Some("Meta+Space,none,Caelestia Launcher")
    );
    ...
    // Caelestia shortcuts REMOVED, but user changes PRESERVED!
    let final_ini = KdeIniFile::parse(&fs::read_to_string(&kglobal_path).unwrap());
    assert_eq!(
        final_ini.get("kwin", "CaelestiaLauncher"),
        None,
        "CaelestiaLauncher must be cleanly removed"
    );
    assert_eq!(
        final_ini.get("kwin", "CaelestiaWallpaper"),
        None,
        "CaelestiaWallpaper must be cleanly removed"
    );
