...
fn check_desktop_environment() -> DependencyCheck {
    ...
    if kwin_path.is_some() || plasma_ver.is_some() {
        ...
        DependencyCheck {
            ...
            required_version: Some("6.0.0".to_string()),
            binary_path: None,
            message: format!("Session type: {}. Neither KWin nor plasmashell was detected in PATH", session),
            recommendation: Some("Astral Plasma is engineered specifically for KDE Plasma 6 and KWin".to_string()),
        }
    }
}
...
fn check_qdbus() -> DependencyCheck {
    ...
    if let Some(p) = path {
        DependencyCheck {
            ...
            binary_path: Some(p),
            message: "qdbus6 binary available for Plasma session and KWin window management".to_string(),
            recommendation: None,
        }
    } else {
        DependencyCheck {
            ...
            required_version: None,
            binary_path: None,
            message: "Neither qdbus6 nor qdbus was found in PATH".to_string(),
            recommendation: Some("Install Qt6 DBus tools: sudo pacman -S qt6-tools".to_string()),
        }
    }
}
